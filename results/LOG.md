...
| M32 | [`m32.md`](log/m32.md) | DONE (adversarial-review-hardened). Splits M31's Δ(nomix−ff) "shared-readout / channel-independent parameterization" bundle into its 3 ingredients + isolates mixing (3 off-by-default TRMMixer controls: `*_nomix_unsharedro`, `*_unsharedro`, `*_nomix_distinctw`; `ff`/`nomix` reproduce M31 bit-identically, `trm_flat` 5/6 cells). **Token-mixing is NOT the mechanism — no help everywhere, net HARMFUL on weather (Δ(mixer−nomix) +0.03→+0.16, decisive 10/0–9/1) but NOT significant on etth1 (8/2, 6/4, 8/2); not monotone in horizon — strengthens M31's "not mixing."** Shared readout = REAL but MODEST helper (−0.03→−0.10, sign-consistent). **Channel-independence is the LARGER structural slice ON THE MANY-CHANNEL dataset** (vs fair `trm_flat` baseline: 76–84% weather M=21) but **small/noise-dominated on etth1 M=7 (~9–44%, hinges on a 0.009 baseline wobble)** — the split is DATASET-dependent, not universal. Weight-sharing a modest third (−0.03→−0.09). Corrects M31's EMPHASIS (readout first) → CI leads on the many-channel side; compound name real (both contribute), M31's FIRM negative strengthened. **Bounds: most of the raw nomix−ff number is ff-OVERFIT not an ingredient (`trm_flat`−ff = 81% of weather-h720 headline); the CI contrast bundles CI+readout-structure+width (not a single-knob flip); one repro wrinkle — `trm_flat` etth1-336 drifts +0.0086, a 2-D arm, NOT the 3-D cause.** Forecasting-only; synthetic mixing wins untouched. |
