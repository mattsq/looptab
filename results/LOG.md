...
| M32 | [`m32.md`](log/m32.md) | DONE. Splits M31's Δ(nomix−ff) "shared-readout / channel-independent parameterization" bundle into its 3 ingredients + isolates mixing (3 new off-by-default TRMMixer controls: `*_nomix_unsharedro`, `*_unsharedro`, `*_nomix_distinctw`; all 6 cells reproduce M31 to rounding). Ordering by contribution: **channel-INDEPENDENCE ≥ shared readout > weight-sharing ≫ mixing (NEGATIVE)**. Token-mixing is net HARMFUL (Δ(mixer−nomix) +0.03→+0.16 in ALL 6, decisive on weather 10/0–9/1, worsens with horizon — strengthens M31 C1). Shared readout is a REAL but MODEST helper (Δ(nomix−nomix_unsh) −0.03→−0.10, sign-consistent). Channel-independence is the LARGER structural ingredient, dominant when channels are many: vs the fair `trm_flat` baseline, CI carries 76–84% on weather (M=21) but 15–44% on etth1 (M=7). Weight-sharing a modest third (−0.03→−0.09). **Corrects M31's EMPHASIS** (readout named first) → CI leads, readout second; M31's compound name is real (both contribute), M31's FIRM negative ("not mixing") strengthened. The nomix−ff headline magnitude is an ff-OVERFIT artifact (`trm_flat` is the honest CD baseline). Forecasting-only; synthetic mixing wins untouched. |
