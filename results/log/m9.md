...
on whole-row exact-match) along §11.3's two named sub-levers: **(ii)** sweep the output width `w` to
...
clean fix (§11.3(i)) is still deferred. (iv) A **decoupled-head ablation** (does the *joint*
